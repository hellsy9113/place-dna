...
function LandmarkFallbackArtwork({
  landmarkName,
  tone,
}: Pick<LandmarkImageProps, "landmarkName" | "tone">) {
  return (
    <>
      <ShapeBadge
        tone={tone}
      ...
      <div
        aria-hidden="true"
        className="mt-14 flex items-end gap-2 print:mt-6 print:gap-1.5"
      >
...
export function LandmarkImage({
  ...
  if (!imageUrl || failed) {
    return (
      <div
        role="img"
        aria-label={`Landmark image unavailable for ${landmarkName}`}
        className="relative h-52 rounded-[1.5rem_1.5rem_1rem_1rem] border-2 border-[color:var(--placedna-ink)] bg-[linear-gradient(135deg,rgba(139,92,246,0.22),rgba(244,114,182,0.16),rgba(251,191,36,0.22),rgba(255,255,255,0.8))] p-4 print:h-28 print:rounded-[1rem_1rem_0.75rem_0.75rem] print:p-2.5"
      >
        <LandmarkFallbackArtwork landmarkName={landmarkName} tone={tone} />
      </div>
    );
  }
